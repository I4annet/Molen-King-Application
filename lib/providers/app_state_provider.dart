...
class AppStateProvider extends ChangeNotifier {
  ...
  Map<String, int> _stocks = {'keju': 100, 'ori': 150, 'coklat': 120};
  ...
  Map<String, int> get stocks => _stocks;
  ...
  Future<void> _initialize() async {
    ...
    if (SupabaseConfig.isConfigured) {
      try {
        // Coba koneksi Supabase
        _isSimulationMode = false;
        await _loadFromSupabase();
      } catch (e) {
        debugPrint("Koneksi Supabase gagal, beralih ke Mode Simulasi: $e");
  ...
  Future<void> _loadFromLocalStorage() async {
    ...
    // Load stocks
    final stocksStr = prefs.getString('sim_stocks');
    if (stocksStr != null) {
      final Map<String, dynamic> decoded = jsonDecode(stocksStr);
      _stocks = decoded.map((key, value) => MapEntry(key, value as int));
    }
    ...
    if (expStr != null) {
      ...
      _expenses = decoded.map((e) => ExpenseModel.fromJson(e as Map<String, dynamic>)).toList();
    }
    ...
    if (attStr != null) {
      ...
      _attendanceLogs = decoded.map((e) => AttendanceModel.fromJson(e as Map<String, dynamic>)).toList();
    }
    notifyListeners();
  }
  ...
  Future<void> _saveCurrentUserToLocal() async {
    ...
    if (_currentUser != null) {
      await prefs.setString('sim_current_user', jsonEncode(_currentUser!.toJson()));
    } else {
      await prefs.remove('sim_current_user');
    }
  }

  Future<void> _saveStocksToLocal() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('sim_stocks', jsonEncode(_stocks));
  }
  ...
  Future<void> _loadFromSupabase() async {
    ...
    // Load stocks
    final stocksRes = await supabase.from('stocks').select();
    if (stocksRes.isNotEmpty) {
      for (var row in stocksRes) {
        final flavor = row['flavor'] as String;
        final qty = row['quantity'] as int;
        _stocks[flavor] = qty;
      }
    } else {
      // Initialize stocks table if empty
      for (var entry in _stocks.entries) {
    ...
    final txsRes = await supabase.from('transactions').select('*, transaction_items(*)').order('created_at', ascending: false);
    ...
    // Load expenses
    final expRes = await supabase.from('expenses').select().order('created_at', ascending: false);
    _expenses = expRes.map<ExpenseModel>((e) => ExpenseModel.fromJson(e)).toList();

    // Load attendance logs
    final attRes = await supabase.from('attendance_logs').select().order('check_in_time', ascending: false);
    _attendanceLogs = attRes.map<AttendanceModel>((e) => AttendanceModel.fromJson(e)).toList();
  ...
  Future<bool> register({
    ...
    try {
      if (_isSimulationMode) {
        // Simulasi Registrasi
        final isExist = _users.any((u) => u.email.toLowerCase() == email.trim().toLowerCase());
        if (isExist) {
  ...
  // --- STOCK MANAGEMENT ---
  Future<bool> updateStock(String flavor, int amountChange) async {
    _setLoading(true);
    try {
      final currentQty = _stocks[flavor] ?? 0;
      final newQty = (currentQty + amountChange).clamp(0, 99999);
      _stocks[flavor] = newQty;
  ...
  Future<bool> createTransaction(List<TransactionItem> items) async {
    ...
    try {
      ...
      if (_isSimulationMode) {
        ...
        final txRes = await supabase.from('transactions').insert({
  ...
  List<MapEntry<String, double>> getChartDataForPeriod(String period) {
    ...
    if (period == 'daily') {
      ...
      for (int i = 0; i < 12; i++) {
        ...
        final end = DateTime(targetYear, monthIndex + 2, 1).subtract(const Duration(seconds: 1));
